...
export default function App() {
  ...
  return <div className="page"><style>{CSS}</style><div className="wrap">
    ...
    <main className="main"><section className="card"><h2>Scaled line parameters</h2><div className="metrics"><Metric label="R" value={fmt(r, 5)} unit="p.u." /><Metric label="X" value={fmt(x, 5)} unit="p.u." /><Metric label="B" value={fmt(b, 5)} unit="p.u." /><Metric label="Length factor" value={fmt(scale, 2)} unit="×100 km" /></div><p className="small">400 kV: R=0.00161, X=0.01729, B=0.66554 p.u./100 km. 220 kV: R=0.00956, X=0.06518, B=0.17790 p.u./100 km.</p></section>
      ...
      <section className="card"><div className="tabs">{[['instructions', 'Instructions'], ['balance', 'Reactive balance'], ['q', 'Q sweep'], ['v', 'Voltage sweep'], ['notes', 'Model notes']].map(([k, text]) => <button key={k} className={tab === k ? 'active' : ''} onClick={() => setTab(k)}>{text}</button>)}</div>
        {tab === 'instructions' && <div className="instructions"><h2>How to use this applet</h2><div className="stepGrid"><div><strong>1. Start with Q sweep</strong><p>Open the Q sweep tab. The zero line marks the transition between net capacitive injection and net inductive absorption.</p></div><div><strong>2. Increase line length</strong><p>Longer lines amplify both series reactance and shunt susceptance. Try 300–800 km to make the effect visible.</p></div><div><strong>3. Vary sending-end voltage</strong><p>Increasing voltage strengthens the V²B charging term. Decreasing voltage weakens capacitive injection.</p></div><div><strong>4. Increase loading</strong><p>Higher load raises current and therefore I²X series reactive absorption, moving the line towards inductive behaviour.</p></div><div><strong>5. Freeze the y-axis</strong><p>Use the fixed-axis option before changing voltage or length so the curve shift is not hidden by automatic rescaling.</p></div><div><strong>6. Compare with voltage sweep</strong><p>The voltage sweep shows the light-load Ferranti region and the heavier-load region where series voltage drop dominates.</p></div></div><p className="small"><strong>Interpretation:</strong> the line parameters do not change sign. The net terminal behaviour changes because the dominant term changes between shunt capacitive injection and series inductive absorption.</p></div>}
